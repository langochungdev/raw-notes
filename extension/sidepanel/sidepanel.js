...
const renderShareMenu = () => {
  ...
  if (!app.activePath) {
    shareMenu.replaceChildren(...new DOMParser().parseFromString(`
...
const renderSlashMenu = () => {
  ...
  slashMenu.replaceChildren(...new DOMParser().parseFromString(options.map((option, index) => {
    return `<button type="button" data-index="${index}" class="slash-item ${index === slashState.index ? "selected" : ""}">
      ${option.label}
    </button>`;
