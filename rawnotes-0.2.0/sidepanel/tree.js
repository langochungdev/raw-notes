...
export const createVaultTree = ({
  ...
  const openMenu = (anchor, entryInfo, labelEl) => {
    ...
    menu.replaceChildren(...new DOMParser().parseFromString(`
  ...
  const renderDragGhost = (entryInfo, x, y) => {
    if (!dragGhost) {
      ...
      dragGhost.replaceChildren(...new DOMParser().parseFromString(`
  ...
  const renderEntry = async (entry, parentHandle, parentPath, depth) => {
    ...
    chevron.replaceChildren(...new DOMParser().parseFromString(createIcon("chevron"), "text/html").body.childNodes);
    if (!isFolder) {
    ...
    icon.replaceChildren(...new DOMParser().parseFromString(createIcon(isFolder ? "folder" : "file"), "text/html").body.childNodes);
    ...
    dragHandle.replaceChildren(...new DOMParser().parseFromString(createIcon("drag"), "text/html").body.childNodes);
    ...
    more.replaceChildren(...new DOMParser().parseFromString(createIcon("dots"), "text/html").body.childNodes);
